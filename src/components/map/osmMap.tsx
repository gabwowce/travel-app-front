...
export default function OSMMap({ title }: OSMMapProps) {
  ...
  const [userLocation, setUserLocation] = useState<{ latitude: number; longitude: number } | null>(defaultLocation);
...
<>
<ExpoStatusBar style={webViewUrl ? "dark" : "light"} translucent backgroundColor="transparent" />
<RNStatusBar barStyle={webViewUrl ? "dark-content" : "light-content"} translucent backgroundColor="transparent" />
{webViewUrl ? (
  <>
    <MapView
          style={styles.map}
    ...
    {!webViewLoading && <BackButton label="Grįžti į žemėlapį" onPress={() => setWebViewUrl(null)} />}
  </>
) : (
  <>
    <MapView
          ...
          {webViewUrl ? (
            <>
              <WebView
                ...
                renderLoading={() => <ActivityIndicator size="large" color="#444" style={{ flex: 1 }} />}
              />
              {!webViewLoading && <BackButton label="Grįžti į žemėlapį" onPress={() => setWebViewUrl(null)} />}
            </>
          ) : (
